...
//    run_simulations_for_each_simple("profile_for_each_seq_simple", suffix, get_inputs_profile_serial_simple(), std::execution::seq);
//    std::cout << "profile_for_each_seq_simple: " << timer.get_elapse_time_s() << " s\n";

    run_simulations_for_each_simple("profile_for_each_par_simple", suffix, get_inputs_profile_serial_simple());
    std::cout << "profile_for_each_par_simple: " << timer.get_elapse_time_s() << " s\n";

//    run_simulations_for_each_simple("profile_for_each_par_unseq_simple", suffix, get_inputs_profile_serial_simple(), std::execution::par_unseq);
//    std::cout << "profile_for_each_par_unseq_simple: " << timer.get_elapse_time_s() << " s\n";

    std::cout << "profile" << ": " << timer.get_total_elapse_time_s() << " s\n";

//    std::cout << decltype(std::execution::seq) << "\n";
}