...
double Rutherford::compute_sigma_nm2(double energy_eV)
{
    double const energy_keV = energy_eV * 1.0e-3;

    double const factor1 = 5.21e-21 * 1.0e14 * (atomic_number * atomic_number) / (energy_keV * energy_keV);
    ...
    double const value = factor1 * factor2 * factor3 * factor3;

    return value;
}

double Rutherford::compute_scattering_angle_rad(double energy_eV, double random_number)
{
    double const alpha = compute_alpha(energy_eV);
